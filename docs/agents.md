...
plus the exit code for each kind of failure. It walks the live command tree, so it cannot drift
from the program. `braze schema <operation> --json` answers the same question for one operation,
